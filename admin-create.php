...
if(isset($_POST['addcourseBtn']))
{
	...
	{	
		...
		header("Refresh:0; url=admin-create.html");	
	}
	else									  // Array does not have duplicates
	{	
		if(empty($_POST['cCRN']) || empty($_POST['cName']) || empty($_POST['cCapacity']) || empty($_POST['cDescription']) || empty($_POST['ins'])
		...
		{
			$check_ins= mysqli_query($link,"SELECT * from instructor WHERE T_username = '$ins'");
			...
			$query = "SELECT CT.Time_D_H FROM ins_course IC,course_time CT WHERE IC.Instructor = '$ins' AND IC.Course = CT.Course ";
			...
			if($row_cnt!=1)
			{
				...
				$res2=mysqli_query($link,"INSERT INTO `ins_course`(Course, Instructor) VALUES ('$cCRN', '$ins')");
